...
from nnunetv2.training.loss.compound_losses import KD_KLDiv_loss
import os
from datetime import datetime
...
class KDTrainer_KLdiv_osm_t2(nnUNetTrainer):
    ...
    def build_teacher_network_architecture(architecture_class_name: str,
        ...
        pretrain_network = torch.load("/data/nnUNet_Dataset/nnUNet_results/Dataset503_Kits19KD_CECT/nnUNetTrainer__nnUNetPlans__3d_fullres/fold_0/checkpoint_final.pth")['network_weights']
    ...
    def _build_t_loss(self):
        ...
        loss = KD_KLDiv_loss(reduction = 'mean', temperature = 2)
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if 0 else dummy_context():
            ...
            student_l = self.loss([student_output], [temp_target])               # DC_CE_loss(student network output, gt label)
            teacher_l = self.t_loss([student_output], [teacher_output])     # KLDiv loss
            
            # Weight needs to be adjusted
            student_weight = 0.75
            teacher_weight = 0.25
            l = student_weight * student_l + teacher_weight * teacher_l
